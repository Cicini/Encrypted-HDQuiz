const HDQ = {
	...
	VARS: {},
	init: async function () {
		console.log("Encrypted-HDQuiz 2023.07.29 Loaded");
	...
	submit: async function (el) {
		...
		// get list of all questions to send to backend
		let dataa = await getQuestionData();
		jQuery.ajax({
			type: "POST",
			data: {
				action: "hdq_validate_answers",
				data: dataa,
			},
			url: HDQ.VARS.ajax,
			success: async function (res) {
				console.log(res)
				res = JSON.parse(res);
				await setAnswerValues(res);
			},
		});
		
		async function setAnswerValues(data){
			// I don't know if you want or need marking
			// but this next loop will make it easier to add the marking/status class
			// for each question and answer
			for(let i = 0; i < data.length; i++){
				if(data[i].answer != ""){
					// create answer dom ID
					data[i].answer -= 1;
					data[i].answer = `hdq_option_${data[i].answer}_${data[i].question}`;
				}
				// create question dom ID
				data[i].question = `hdq_question_${data[i].question}`;
			}			
			console.log(data); 
			
			// get the final score
			let score = [0, data.length];
			let correct = 0;
			for(let i = 0; i < data.length; i++){
			...
			if (jQuery(".hdq_results_inner .hdq_result .hdq_result_percent")[0]) {
				...
				html = '<span class = "hdq_result_fraction">' + html + '</span> - <span class = "hdq_result_percent">' + hdq_results_percent + "%</span>";
			}
			jQuery(".hdq_results_inner .hdq_result").html(html);
		...
		async function getQuestionData(){
			...
			for(let i = 0; i < questions.length; i++){
				let id = parseInt(questions[i].getAttribute("id").replace("hdq_question_", ""));
				let answer = null;
				if(questions[i].getAttribute("data-type") !== "text_based"){
					let answers = questions[i].getElementsByClassName("hdq_option");
					for(let i = 0; i < answers.length; i++){
						if(answers[i].checked){
							answer = answers[i].getAttribute("id").split("_")[2];
							break;
						}
					}
				} else {
					// text based answers
					answer = questions[i].getElementsByClassName("hdq_option")[0].value;
				}
